...
async function loginDiscordActivity() {
  ...
  const userAccessToken = await exchangeCodeForTokenViaApi(code, state)
  dLog("login", "exchange_ok", { loginId, tokenLen: userAccessToken.length })

  let auth = null
  try {
    const authStarted = dMsNow()
    dLog("login", "authenticate_call", { loginId })
    auth = await discordSdk.commands.authenticate({ access_token: userAccessToken })
    dLog("login", "authenticate_ok", { loginId, ms: Math.round(dMsNow() - authStarted), hasUser: Boolean(auth?.user) })
  } catch (e) {
